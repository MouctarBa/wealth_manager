...
const routes = [
  ...
  { name: 'Budgeting', path: '/budgeting', Icon: FaRegCalendarAlt },
...
export default function Sidebar() {
  ...
  return (
    <aside className="w-72 bg-white border-r border-gray-200 p-6 h-full">
        ...
        {routes.map(({ name, path, Icon }) => (
          <Link
            ...
            className={`flex items-center py-2 px-3 rounded-lg transition-colors ${
              current === path ? 'bg-blue-100 text-blue-600' : 'hover:bg-gray-100'
            }`}
      ...
      <button
        ...
        aria-label="Sign out"
      >
        <FaHome className="mr-2 rotate-180" />
        Sign Out
      </button>
    </aside>
  );
